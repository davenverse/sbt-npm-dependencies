...
/**
  ...
  * in the artifact .jar.
  * 
  * We need to read this information from other deps to
  * build a correct deps for our package
  */
...
object NpmDependencies {
  ...
  implicit val decoder = Decoder.instance[NpmDependencies](h => 
      (
        h.downField("compile-dependencies").as[List[TupledMap[String]]].map(_.map(_.tupled)),
        h.downField("test-dependencies").as[List[TupledMap[String]]].map(_.map(_.tupled)),
        h.downField("compile-devDependencies").as[List[TupledMap[String]]].map(_.map(_.tupled)),
        h.downField("test-devDependencies").as[List[TupledMap[String]]].map(_.map(_.tupled))
  ...
  def writeManifest(
    ...
    IO.write(manifestFile, Encoder[NpmDependencies].apply(npmDependencies).noSpaces)
    manifestFile
  }
